...
const MonitoringDashboardPage = () => {
  ...
  const programmeSummaryQuery = useQuery({
    queryKey: ["analytics", "programme", "monitoring"],
    queryFn: getProgrammeSummary,
  });

  const institutionsQuery = useQuery({
    queryKey: ["analytics", "institutions", "monitoring", institutionsPage, PAGE_LIMIT],
    queryFn: () => getAnalyticsInstitutions({ page: institutionsPage, limit: PAGE_LIMIT }),
  });

  const institutionSummaryQuery = useQuery({
    queryKey: ["analytics", "institution", selectedInstitutionId, "monitoring"],
  ...
  const selectedInstitutionName =
    institutionsQuery.data?.items.find((institution) => institution.id === selectedInstitutionId)?.name ??
    "institution";
  ...
  return (
    <div className="space-y-6">
      <Card id="global-overview" className="scroll-mt-24">
        <SectionHeader
          title="Monitoring Dashboard"
          subtitle="Read-only programme oversight with global metrics and institution inspection."
        />
      ...
      <Card id="metrics" className="scroll-mt-24">
        <SectionHeader title="Global Operational Overview" subtitle="All institutions combined, no write actions." />
        {programmeSummaryQuery.isPending ? <p className="muted-text">Loading programme summary...</p> : null}
        {programmeSummaryQuery.isError ? (
          <p className="text-sm text-rose-300">Failed to load summary: {programmeSummaryQuery.error.message}</p>
        ) : null}
        {programmeSummaryQuery.data ? (
          <div className="space-y-4">
            <SummaryGrid
              items={programmeSummaryItems}
              renderBadge={(badge) => <MetricBadge label={badge.label} tone={badge.tone} />}
            />
            <AttendanceOverview
              title="Programme Attendance"
              subtitle="Presence and absence across all tracked sessions."
      ...
      <Card id="institutions" className="scroll-mt-24">
        <SectionHeader
          ...
          subtitle="Select one institution to inspect its read-only performance metrics."
        />
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          />
          <div>
            <label htmlFor="monitoring-institution" className="field-label">
              Institution
            </label>
            <Select
              id="monitoring-institution"
        ...
        (institutionsQuery.data?.items.length ?? 0) === 0 ? (
          <p className="muted-text mt-3">No institutions available for inspection.</p>
        ) : null}
        {!institutionsQuery.isPending && !institutionsQuery.isError ? (
          <div className="mt-4">
            <DataTable
              ...
              getMobileSubtitle={() => "Read-only inspection"}
        ...
        {selectedInstitutionId.length > 0 && institutionSummaryQuery.isPending ? (
          <p className="muted-text mt-4">Loading institution summary...</p>
        ) : null}
        {selectedInstitutionId.length > 0 && institutionSummaryQuery.isError ? (
          <p className="mt-4 text-sm text-rose-300">
            Failed to load institution summary: {institutionSummaryQuery.error.message}
          </p>
        ) : null}
        {institutionSummaryQuery.data ? (
          <div className="mt-4 space-y-4">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-medium text-slate-200">Inspecting {selectedInstitutionName}</p>
              <MetricBadge
              ...
              items={[
                {
                  ...
                  helperText: "Batches under institution",
                },
                {
                  title: "Students",
                  value: String(institutionSummaryQuery.data.totalStudents),
                  helperText: "Institution enrollment",
                },
                {
                  title: "Sessions",
            ...
            <AttendanceOverview
              ...
              subtitle="Presence and absence for selected institution."
...
export { MonitoringDashboardPage };
